...
class RBF_MLPQFunction(QFunction):
    """
...
class Net(nn.Module):
    ...
    def __init__(self, state_dim, n_actions, layers=None):
        ...
        if not self.single_layer:
            self.input_layer = RBFLayer(state_dim, layers[0])
            self.hidden_layers = []
            for l in range(len(layers)-1):
                self.hidden_layers.append(RBFLayer(layers[l],layers[l+1]))
                self.add_module("hidden_layer" + str(l), self.hidden_layers[-1])
            self.output_layer = BatchedWeightedLinear(layers[-1], n_actions)
        else:
            self.input_layer = BatchedWeightedLinear(state_dim, n_actions, bias=False)
...
class RBFLayer(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.centers = nn.Parameter(torch.Tensor(self.out_features, self.in_features))
        self.betas = nn.Parameter(torch.Tensor(1, self.out_features))
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, -1).unsqueeze(1).repeat(1, self.out_features, 1)
        c = self.centers.view(self.out_features,-1).repeat(batch_size, 1, 1)

        return torch.exp(- self.betas * (x-c).pow(2).sum(-1, keepdim=False) )
