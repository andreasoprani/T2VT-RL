...
class DiscreteMLPQFunction(QFunction):
    """
    A Multi Layer Perceptron Q-function that handles non-linear functions
    """
    def __init__(self, state_dim, n_actions, layers=(32,), n_bins=50, initial_params=None):
        self._nn = Net(state_dim, n_actions, layers, n_bins)
    ...
    def init_weights(self):
        ...
        # Setting weights to zero prevents learning
        self._w = np.random.randn(self._nn.n_weights) * 0.1
...
class Net(nn.Module):
    ...
    def __init__(self, state_dim, n_actions, layers=None, n_bins=50):
        super(Net, self).__init__()
        self.double()
        self.single_layer = layers is None or len(layers) == 0
        if not self.single_layer:
            #self.input_layer = BatchedWeightedLinear(state_dim, layers[0])
            self.input_layer = DiscreteBinsLayer(state_dim, layers[0], n_bins=n_bins)
            self.hidden_layers = []
            ...
            #self.output_layer = DiscreteBinsLayer(layers[-1], n_actions, n_bins=n_bins)
            self.output_layer = BatchedWeightedLinear(layers[-1], n_actions)
        else:
            self.input_layer = DiscreteBinsLayer(state_dim, n_actions, n_bins=n_bins)
...
class DiscreteBinsLayer(nn.Module):
    def __init__(self, in_features, out_features, n_bins=50, domain = (-1,1), bias=True):
        super(DiscreteBinsLayer, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.n_bins = n_bins
        self.weight = nn.Parameter(torch.Tensor(self.out_features, self.in_features, self.n_bins))
        self.domain = domain
        self.bin_width = (self.domain[1] - self.domain[0]) / self.n_bins
        self._filter = [(self.domain[0] + i * self.bin_width, self.domain[0] + (i + 1) * self.bin_width, ) for i in range(n_bins)]

        self.weight_batch = 1 # size of the current batch for the weights
        self.double()

    def get_mask(self, x, in_feature):

        mask = torch.zeros(len(self._filter), dtype=torch.double)

        for i, (l, u) in enumerate(self._filter):
            if x[in_feature] >= l and x[in_feature] <= u:
                mask[i] = 1
                break
    ...
    def forward(self, x):
        ...
        #indices = torch.floor((x - self.domain[0]) / self.bin_width)
        #indices = torch.clamp(indices, 0, self.n_bins - 1)

        if x.ndim == 1:
            x = [x]

        out = torch.zeros((len(x), self.out_features), dtype=torch.double)

        for j in range(len(x)):
            for o in range(self.out_features):
                for i in range(self.in_features):
                    mask = self.get_mask(x[j], i)
                    out[j, o] = torch.dot(self.weight[o, i, :], mask)
