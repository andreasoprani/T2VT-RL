...
class TradingProportional(TradingMain):
    ...
    def _normalize(self, X):
        return (X-self.mean_props)/self.std_props

    def _denormalize(self, X):
        return X*self.std_props+self.mean_props
    ...
    def _reward(self):
        ...
        return self.current_portfolio * (self.current_price - self.previous_price)/self.previous_price - \
                abs(self.current_portfolio - self.previous_portfolio) * self.fees

    def reset(self):
        super().reset()
        # Compute derivatives once for all, pad with time_lag zeros
        self.derivatives = self._normalize(np.insert((self.prices[1:] - self.prices[:-1])/self.prices[:-1],0,0))
...
class TradingDerivativesWithStateReward(TradingMain):
